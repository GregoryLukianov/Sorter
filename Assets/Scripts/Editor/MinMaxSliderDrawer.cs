...
internal class MinMaxSliderDrawer : PropertyDrawer
{
    ...
    private float FlexibleFloatFieldWidth(float min, float max)
    {
        ...
        return 30f + (Mathf.Floor(Mathf.Log10(Mathf.Abs(n)) + 1) * 2.5f);
    }
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (attr.DataFields)
        {
            ...
            EditorGUI.BeginChangeCheck();
            EditorGUI.indentLevel = 0;
            string minStr = string.Format("{0:0.00}", min);
            minStr = EditorGUI.DelayedTextField(minPos, minStr);
            EditorGUI.indentLevel = indent;
            if (EditorGUI.EndChangeCheck())
            {
            ...
            EditorGUI.BeginChangeCheck();
            EditorGUI.indentLevel = 0;
            string maxStr = string.Format("{0:0.00}", max);
            maxStr = EditorGUI.DelayedTextField(maxPos, maxStr);
            EditorGUI.indentLevel = indent;
            if (EditorGUI.EndChangeCheck())
            {
                if (float.TryParse(maxStr, out float newMax))
                {
                    max = newMax;
                    if (attr.Bound)
                    {
